...
int log_base2(int y)
//calcul la premire puissance de 2 plus grande que y. Servira pour connaitre la taille qu'il faut donner au pointeur factorisation
{
	int prelog=0;
	while( ((y>>(31-prelog))&1) == (0&1) )
	{
		prelog++;
	}
	//y<2^(32-prelog)
...
{
	int *pointeur;
	int nb_facteurs=log_base2(p)+1;//on rajoute le zero, comme le veut camille
