...
void premier_coeff_non_nul(int tab[(2*m)+1][m+1],int decalagei,int decalagej,int p)
//modifie tab en échangeant lignes entre elles pour que le coef a_1_1 soit non nul.
{
	...
	if (ligne_pour_echange>(2*m))
		printf("Pivot de Gauss a trouvé une colonne vide: p=%d \n",p);
}
...
void Pivot_Gauss_recurrence(int tab[(2*m)+1][m+1],int decalagei,int decalagej,int p,int *delta)
{
	*delta=premiere_colonne_non_nulle(tab,decalagei,decalagej);
	premier_coeff_non_nul(tab,decalagei,decalagej + *delta,p);
...
void remplissage_chinois(int taille_facteur,int chinois[m][taille_facteur],int *facteur,int tab[(2*m)+1][m+1])
{
	...
	for(k=0;k<taille_facteur;k++)
	{
		...
		for(i=0;i<m;i++)
		{	
			if(test_nul_ligne(reduit_modulo,j)==1)
			{
				pivot=place_pivot(reduit_modulo,j);
				if (pivot!=i)
				{
					chinois[i][k]=0;
				}
...
int* theoreme_chinois_log(int taille_facteur,int chinois[m][taille_facteur],int *facteur,int p)
{
	...
	for(i=0;i<m;i++)
	{
		log[i]=(theoreme_chinois(chinois[i],facteur,p) +(p-1) )%(p-1);
	}
...
void afficher_ligne_partielle(int a,int b, int tab[a][b],int i, int j,int decalage,const char * nom)
...
void afficher_colonne_partielle(int a,int b, int tab[a][b],int i, int j,int decalage,const char * nom)
//affiche le tableau de la colonne i à la colonne j en prenant compte du décalage
//attention, les lignes sont comptées à partir de 0.
// j>=i
{
	int colonne;
	int k;
	for(colonne=0;colonne<=(j-i);colonne++)
	{
		printf(nom);
		printf("colonne %d = ",i +colonne);
		for(k=0;k<(a-decalage);k++)
		{
			printf("%d, ",tab[k+decalage][decalage+i+colonne]);
		}
		printf("\n");
	}
	printf("\n");	
...
int bon_resultat(int p,int alpha,int base[m][2])
{
	...
	for(k=0;k<m;k++)
	{
		if (base[k][1]!=0)
		{
			...
			{
				//printf("bon resultat =0 : square_and_multiply = %d, base = %d\n",square_and_multiply(base[k][1],alpha,p) , (base[k][0]%p));
				drapeau=0;
			}
		}
		else
		{
			drapeau=0;
			printf("pas assez de relations indépendantes liant les éléments de la base\n");
		}
	}
	return(drapeau);
}

void retour_zero(int a, int b,int tableau[a][b])
//remet à zero les cases de tableau
{
	int i;
	int j;
	for (i=0;i<a;i++)
...
void recherche_erreur()
{
	...
	if (verification_facteur_simple(facteur_simple)==1)
	{
		...
		for(k=0;k<taille_facteur;k++)
		{
			...
			reduction_mod(tab,reduit_modulo,rho);
			Pivot_Gauss(reduit_modulo,rho);
			afficher_tableau_classique((2*m)+1,m+1,reduit_modulo,"tab mod");
			j=0;
			for(i=0;i<m;i++)
			{	
				//printf("test ligne nulle= %d\n",test_nul_ligne(reduit_modulo,j));
				if(test_nul_ligne(reduit_modulo,j)==1)
				{
					pivot=place_pivot(reduit_modulo,j);
					if (pivot!=i)
			...
			afficher_colonne_partielle(m,taille_facteur,chinois,k,k,0,"chinois");
		}
		
		afficher_tableau_classique(m,taille_facteur,chinois,"chinois");
		loG=theoreme_chinois_log(taille_facteur,chinois,facteur,p);
		afficher_liste_classique(m,loG,"log");
		free(facteur);
